...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		//print(tinypic);
		//print(image);
		//print(blend(tinypic, image, 0.25));
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < newImage.length; i++){
			for (int j = 0; j < newImage[i].length; j++){
				int iIndex = (int) ((double) ((double) image.length / height) * i);
				int jIndex = (int) ((double) ((double) image[0].length / width) * j);
				newImage[i][j] = image[iIndex][jIndex];
			}
		} 
		return newImage;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		int red = (int) ((double) (c1.getRed() * alpha) + (c2.getRed() * (1 - alpha)));
		int green = (int) ((double) (c1.getGreen() * alpha) + (c2.getGreen() * (1 - alpha)));
		int blue = (int) ((double) (c1.getBlue() * alpha) + (c2.getBlue() * (1 - alpha)));
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		// checks sizes of the two images
		...
		for (int i = 0; i <= n; i++){
			...
			Color[][] currentImage = blend(source, target, alpha);

			// only on first time
			if (i == 0){
			setCanvas(currentImage);
			}
